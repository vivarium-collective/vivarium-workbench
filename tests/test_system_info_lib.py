...
class TestServerShimParity:
    ...
    def test_github_repo_parity(self, monkeypatch, ws: Path) -> None:
        from vivarium_dashboard.lib.system_info import build_github_repo
        captured = self._invoke_handler(monkeypatch, ws, "_get_github_repo")
        assert captured["status"] == 200
        assert captured["body"] == build_github_repo(ws)

    def test_ui_config_parity(self, monkeypatch, ws: Path) -> None:
        from vivarium_dashboard.lib.system_info import build_ui_config
        captured = self._invoke_handler(monkeypatch, ws, "_get_ui_config")
        assert captured["status"] == 200
        assert captured["body"] == build_ui_config(ws)

    def test_framework_metrics_parity(self, monkeypatch, ws: Path) -> None:
        """_framework_metrics_test returns same body as build_framework_metrics."""
        import json
        import vivarium_dashboard.server as server
        monkeypatch.setattr(server, "WORKSPACE", ws)
        from vivarium_dashboard.lib.system_info import build_framework_metrics
        body_bytes, code = server.Handler._framework_metrics_test(ws)
        assert code == 200
        assert json.loads(body_bytes) == build_framework_metrics(ws)

    def test_workspace_home_parity(self, monkeypatch, ws: Path) -> None:
        import vivarium_dashboard.server as server
        monkeypatch.setattr(server, "WORKSPACE", ws)
        from vivarium_dashboard.lib.system_info import build_workspace_home
        # _build_api_workspace_response returns (json_bytes, 200)
        import json
        body_bytes, code = server.Handler._build_api_workspace_response()
        assert code == 200
        assert json.loads(body_bytes) == build_workspace_home(ws)
