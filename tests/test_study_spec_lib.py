...
class TestServerShimParity:
    """The legacy stdlib ``_study_detail_spec`` delegates to the lib loader, so
    its output must equal the lib loader's on the same fixture (with the same
    WORKSPACE plumbing)."""

    def test_detail_spec_parity(self, monkeypatch: Any, tmp_path: Path) -> None:
        ws = _make_workspace(tmp_path)
        import vivarium_dashboard.server as server
        monkeypatch.setattr(server, "WORKSPACE", ws)
        legacy = server._study_detail_spec("my-study")
        lib_spec = study_spec.load_study_detail_spec(ws, "my-study")
        assert legacy == lib_spec

    def test_detail_spec_parity_missing(self, monkeypatch: Any, tmp_path: Path) -> None:
        import vivarium_dashboard.server as server
        monkeypatch.setattr(server, "WORKSPACE", tmp_path)
        assert server._study_detail_spec("nope") is None
        assert study_spec.load_study_detail_spec(tmp_path, "nope") is None

    def test_runs_db_shim_parity(self, monkeypatch: Any, tmp_path: Path) -> None:
        ws = _make_workspace(tmp_path)
        import vivarium_dashboard.server as server
        monkeypatch.setattr(server, "WORKSPACE", ws)
        # invalidate the WorkspacePaths cache so the patched WORKSPACE is used
        server._WP_CACHE.clear()
        assert server._read_runs_db_for_study("my-study") == \
            study_spec.read_runs_db_for_study(ws, "my-study")

    def test_discover_viz_shim_parity(self, monkeypatch: Any, tmp_path: Path) -> None:
        ws = _make_workspace(tmp_path)
        viz = ws / "studies" / "my-study" / "viz"
        viz.mkdir()
        (viz / "chart.html").write_text("<html></html>")
        ...
        monkeypatch.setattr(server, "WORKSPACE", ws)
        server._WP_CACHE.clear()
        assert server._discover_viz_html_files("my-study") == \
            study_spec.discover_viz_html_files(ws, "my-study")
