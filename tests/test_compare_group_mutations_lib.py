...
# ---------------------------------------------------------------------------
# Behavioral commit-path tests — monkeypatch server._commit_or_run to a recorder
# ---------------------------------------------------------------------------


class TestServerCommitPath:
    """The 4 live server shims STILL route through ``_commit_or_run``.

    Each test drives the REAL ``server.Handler._post_*`` method with
    ``_commit_or_run`` monkeypatched to a recorder, so we PROVE routing and
    ordering rather than string-matching source code.  Exact commit_msg strings
    are verified verbatim.
    """

    def _handler(self):
        """Construct a bare Handler (no socket) with _json capture."""
        from vivarium_dashboard.server import Handler  # type: ignore[attr-defined]

        handler = object.__new__(Handler)
        captured: dict = {}

        def _capture(data, code=200):
            captured["data"] = data
            captured["code"] = code
            return data

        handler._json = _capture  # type: ignore[attr-defined]
        return handler, captured

    def _make_recorder(self):
        """Return a _commit_or_run-compatible recorder and its call-log."""
        calls: dict = {}

        def _record(commit_message, action_fn):
            calls["commit_msg"] = commit_message
            # Call the action so file side-effects + re-raise behaviour happens
            action_fn()
            return {"ok": True, "note": "recorded"}, 200

        return _record, calls

    # ---- comparison_add ----

    def test_comparison_add_routes_through_commit_or_run(self, ws, monkeypatch):
        import vivarium_dashboard.server as _server

        monkeypatch.setattr(_server, "WORKSPACE", ws)
        recorder, calls = self._make_recorder()
        monkeypatch.setattr(_server, "_commit_or_run", recorder)

        handler, captured = self._handler()
        handler._post_investigation_comparison_add({
            "investigation": "demo",
            "name": "rate-cmp",
            "variants": ["baseline"],
            "observables": ["x"],
        ...
        assert "commit_msg" in calls, "_commit_or_run was NOT called"
        assert calls["commit_msg"] == "feat(investigations/demo): add comparison rate-cmp"
        assert captured["code"] == 200
        assert "note" in captured["data"]
        # File was actually mutated
        assert _read_spec(ws)["comparisons"][0]["name"] == "rate-cmp"

    def test_comparison_add_validation_before_wrapper(self, ws, monkeypatch):
        import vivarium_dashboard.server as _server
        ...
        monkeypatch.setattr(_server, "_commit_or_run", _boom)
        handler, captured = self._handler()
        handler._post_investigation_comparison_add({"investigation": "demo"})
        assert captured["code"] == 400

    def test_comparison_add_do_action_reraises_on_lib_non_200(self, ws, monkeypatch):
        import vivarium_dashboard.server as _server

        monkeypatch.setattr(_server, "WORKSPACE", ws)
        monkeypatch.setattr(_server._compare_grp_mut, "comparison_add",
                            lambda w, b: ({"error": "boom"}, 500))

        captured_action: dict = {}

        def _record_and_run(msg, fn):
            try:
                fn()
        ...
        monkeypatch.setattr(_server, "_commit_or_run", _record_and_run)
        ...
        assert "raised" in captured_action, "do_action() swallowed the lib non-200"
        assert captured["code"] == 500

    # ---- comparison_update ----

    def test_comparison_update_routes_through_commit_or_run(self, ws, monkeypatch):
        import vivarium_dashboard.server as _server

        # Seed a comparison first
        cgm.comparison_add(ws, {
        ...
        handler, captured = self._handler()
        handler._post_investigation_comparison_update({
            "investigation": "demo",
            "name": "c1",
            "fields_to_update": {"description": "updated"},
        })

        assert calls["commit_msg"] == "feat(investigations/demo): update comparison c1"
        assert captured["code"] == 200
        assert "note" in captured["data"]

    def test_comparison_update_validation_before_wrapper(self, ws, monkeypatch):
        import vivarium_dashboard.server as _server
        ...
        monkeypatch.setattr(_server, "_commit_or_run", _boom)
        handler, captured = self._handler()
        handler._post_investigation_comparison_update({"name": "c1", "fields_to_update": {}})
        assert captured["code"] == 400

    def test_comparison_update_do_action_reraises_on_lib_non_200(self, ws, monkeypatch):
        import vivarium_dashboard.server as _server

        monkeypatch.setattr(_server, "WORKSPACE", ws)
        monkeypatch.setattr(_server._compare_grp_mut, "comparison_update",
                            lambda w, b: ({"error": "boom"}, 500))
        ...
        handler, captured = self._handler()
        handler._post_investigation_comparison_update({
            "investigation": "demo", "name": "c1", "fields_to_update": {},
        })
        assert "raised" in captured_action, "do_action() swallowed the lib non-200"
        assert captured["code"] == 500
    ...
    def test_group_add_routes_through_commit_or_run(self, ws, monkeypatch):
        import vivarium_dashboard.server as _server
        ...
        handler, captured = self._handler()
        handler._post_investigation_group_add({
            "investigation": "demo",
            "name": "control",
            "variants": ["baseline"],
        })

        assert calls["commit_msg"] == "feat(investigations/demo): add group control"
        assert captured["code"] == 200
        assert "note" in captured["data"]
        assert _read_spec(ws)["groups"][0]["name"] == "control"

    def test_group_add_validation_before_wrapper(self, ws, monkeypatch):
        import vivarium_dashboard.server as _server
        ...
        monkeypatch.setattr(_server, "_commit_or_run", _boom)
        handler, captured = self._handler()
        handler._post_investigation_group_add({"investigation": "demo"})
        assert captured["code"] == 400

    def test_group_add_do_action_reraises_on_lib_non_200(self, ws, monkeypatch):
        import vivarium_dashboard.server as _server

        monkeypatch.setattr(_server, "WORKSPACE", ws)
        monkeypatch.setattr(_server._compare_grp_mut, "group_add",
                            lambda w, b: ({"error": "boom"}, 500))
    ...
    def test_group_update_routes_through_commit_or_run(self, ws, monkeypatch):
        import vivarium_dashboard.server as _server
        ...
        handler, captured = self._handler()
        handler._post_investigation_group_update({
            "investigation": "demo",
            "name": "ctrl",
            "fields_to_update": {"description": "updated"},
        })

        assert calls["commit_msg"] == "feat(investigations/demo): update group ctrl"
        assert captured["code"] == 200
        assert "note" in captured["data"]

    def test_group_update_validation_before_wrapper(self, ws, monkeypatch):
        import vivarium_dashboard.server as _server
        ...
        monkeypatch.setattr(_server, "_commit_or_run", _boom)
        handler, captured = self._handler()
        handler._post_investigation_group_update({"name": "ctrl", "fields_to_update": {}})
        assert captured["code"] == 400

    def test_group_update_do_action_reraises_on_lib_non_200(self, ws, monkeypatch):
        import vivarium_dashboard.server as _server

        monkeypatch.setattr(_server, "WORKSPACE", ws)
        monkeypatch.setattr(_server._compare_grp_mut, "group_update",
                            lambda w, b: ({"error": "boom"}, 500))
