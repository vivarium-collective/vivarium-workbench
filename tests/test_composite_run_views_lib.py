...
class TestCompositeRunServerShimParity:
    """Invoke the real ``server.Handler`` shim methods and assert they match the
    lib builder output (same ``(body, status)`` pair).

    Follows the ``TestExplorerServerShimParity`` pattern: construct a
    ``server.Handler`` via ``__new__``, patch ``_json`` to capture the call,
    set ``handler.path``, and invoke the method.
    """

    @staticmethod
    def _invoke_shim(monkeypatch, ws_root: Path, method: str, path: str) -> dict:
        import vivarium_dashboard.server as server

        monkeypatch.setattr(server, "WORKSPACE", ws_root)
        # WorkspacePaths.load uses WORKSPACE transitively inside the lib.
        # The shim now imports from composite_run_views which calls
        # WorkspacePaths.load(WORKSPACE), so patching WORKSPACE is sufficient.
    ...
    def test_composite_runs_missing_spec_id(self, tmp_path, monkeypatch):
        ws = _make_ws(tmp_path)
        lib_body, lib_status = build_composite_runs(ws, None)
        cap = self._invoke_shim(
            monkeypatch, ws, "_get_composite_runs", "/api/composite-runs"
        )
        assert cap["status"] == lib_status == 400
        assert cap["body"] == lib_body

    def test_composite_runs_no_db(self, tmp_path, monkeypatch):
        ws = _make_ws(tmp_path)
        lib_body, lib_status = build_composite_runs(ws, "demo.spec")
        cap = self._invoke_shim(
            monkeypatch, ws, "_get_composite_runs",
            "/api/composite-runs?spec_id=demo.spec",
        )
        assert cap["status"] == lib_status == 200
        assert cap["body"] == lib_body

    def test_composite_runs_seeded(self, tmp_path, monkeypatch):
        ws = _make_ws(tmp_path)
        _seed_db(ws, spec_id="demo.spec", run_id="r1")
        lib_body, lib_status = build_composite_runs(ws, "demo.spec")
        cap = self._invoke_shim(
    ...
    def test_composite_run_no_db(self, tmp_path, monkeypatch):
        ws = _make_ws(tmp_path)
        lib_body, lib_status = build_composite_run(ws, "r1")
        cap = self._invoke_shim(
            monkeypatch, ws, "_get_composite_run", "/api/composite-run/r1"
        )
        assert cap["status"] == lib_status == 404
        assert cap["body"] == lib_body

    def test_composite_run_seeded(self, tmp_path, monkeypatch):
        ws = _make_ws(tmp_path)
        db = _seed_db(ws, run_id="r1")
        _seed_history(db, "r1", 0, {"v": 7})
        lib_body, lib_status = build_composite_run(ws, "r1")
        cap = self._invoke_shim(
            monkeypatch, ws, "_get_composite_run", "/api/composite-run/r1"
        )
    ...
    def test_composite_run_state_bad_step(self, tmp_path, monkeypatch):
        ws = _make_ws(tmp_path)
        _seed_db(ws, run_id="r1")
        cap = self._invoke_shim(
            monkeypatch, ws, "_get_composite_run_state",
            "/api/composite-run/r1/state?step=bad",
        )
        assert cap["status"] == 400
        assert cap["body"] == {"error": "step must be int"}

    def test_composite_run_state_no_db(self, tmp_path, monkeypatch):
        ws = _make_ws(tmp_path)
        lib_body, lib_status = build_composite_run_state(ws, "r1", 0)
        cap = self._invoke_shim(
            monkeypatch, ws, "_get_composite_run_state",
            "/api/composite-run/r1/state?step=0",
        )
        assert cap["status"] == lib_status == 404
        assert cap["body"] == lib_body

    def test_composite_run_state_seeded(self, tmp_path, monkeypatch):
        ws = _make_ws(tmp_path)
        db = _seed_db(ws, run_id="r1")
        _seed_history(db, "r1", 2, {"y": 3.14})
        lib_body, lib_status = build_composite_run_state(ws, "r1", 2)
        cap = self._invoke_shim(
            monkeypatch, ws, "_get_composite_run_state",
            "/api/composite-run/r1/state?step=2",
        )
        assert cap["status"] == lib_status == 200
        assert cap["body"] == lib_body

    def test_composite_run_status_no_db(self, tmp_path, monkeypatch):
        ws = _make_ws(tmp_path)
        lib_body, lib_status = build_composite_run_status(ws, "r1")
        cap = self._invoke_shim(
            monkeypatch, ws, "_get_composite_run_status",
            "/api/composite-run/r1/status",
        )
        assert cap["status"] == lib_status == 404
        assert cap["body"] == lib_body

    def test_composite_run_status_seeded(self, tmp_path, monkeypatch):
        ws = _make_ws(tmp_path)
    ...
    def test_composite_run_status_with_viz(self, tmp_path, monkeypatch):
        ws = _make_ws(tmp_path)
        run_id = "r1"
        _seed_db(ws, run_id=run_id, status="completed")
        viz_dir = ws / ".pbg" / "runs" / run_id
        viz_dir.mkdir(parents=True)
        (viz_dir / "viz.json").write_text(json.dumps({"k": "v"}), encoding="utf-8")

        lib_body, lib_status = build_composite_run_status(ws, run_id)
        cap = self._invoke_shim(
            monkeypatch, ws, "_get_composite_run_status",
            f"/api/composite-run/{run_id}/status",
        )
        assert cap["status"] == lib_status == 200
        assert cap["body"] == lib_body
        assert cap["body"]["viz_html"] == {"k": "v"}
