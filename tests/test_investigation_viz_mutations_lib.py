...
# ---------------------------------------------------------------------------
# Behavioral commit-path tests (drive real server shims)
# ---------------------------------------------------------------------------
...
class TestAddVizCommitPath:
    def test_commit_msg_and_augmentation(self, ws: Path, monkeypatch: Any) -> None:
        import vivarium_dashboard.server as _srv
        monkeypatch.setattr(_srv, "WORKSPACE", ws)
        inv = _make_inv(ws, {"name": _INV})
        calls: dict[str, Any] = {}
        monkeypatch.setattr(_srv, "_active_branch_action", _recorder_factory(calls))
        handler, captured = _make_handler_and_capture()
        handler._post_investigation_add_viz({
            "investigation": _INV, "name": "p", "address": "local:X", "config": {"a": 1},
        })
        assert calls["commit_msg"] == (
            f"feat(investigations/{_INV}): add viz p (local:X)"
        )
        assert captured["code"] == 200
        # Post-wrapper augmentation on the live path.
        assert captured["resp"]["ok"] is True
        assert captured["resp"]["investigation"] == _INV
        assert captured["resp"]["viz_name"] == "p"
        assert _read(inv / "spec.yaml")["visualizations"][0]["name"] == "p"
    ...
    def test_400_before_wrapper(self, ws: Path, monkeypatch: Any) -> None:
        ...
        handler, captured = _make_handler_and_capture()
        handler._post_investigation_add_viz({"investigation": _INV, "name": "p"})
        assert captured["code"] == 400

    def test_duplicate_collapses_to_500_on_live(self, ws: Path, monkeypatch: Any) -> None:
        # The live action() raises RuntimeError on duplicate; the real
        # _active_branch_action catches it → 500 (byte-identical to before).
        import vivarium_dashboard.server as _srv
        monkeypatch.setattr(_srv, "WORKSPACE", ws)
        _make_inv(ws, {"name": _INV, "visualizations": [
            {"name": "p", "address": "local:X", "config": {}},
        ...
        def _run(commit_message, action_fn):
            try:
                action_fn()
        ...
        monkeypatch.setattr(_srv, "_active_branch_action", _run)
        handler, captured = _make_handler_and_capture()
        handler._post_investigation_add_viz({
            "investigation": _INV, "name": "p", "address": "local:Y",
        })
        assert captured["code"] == 500
        assert "already exists in spec" in captured["resp"]["error"]


class TestRenderVizDelegation:
    def test_shim_delegates_to_lib(self, ws: Path, monkeypatch: Any) -> None:
        import vivarium_dashboard.server as _srv
        monkeypatch.setattr(_srv, "WORKSPACE", ws)
        sentinel = ({"ok": True, "sentinel": 1}, 200)
        seen: dict[str, Any] = {}

        def _fake_render(ws_root, body):
            seen["ws_root"] = ws_root
            seen["body"] = body
            return sentinel

        monkeypatch.setattr(_srv._inv_viz_mut, "render_viz", _fake_render)
        handler, captured = _make_handler_and_capture()
        handler._post_investigation_render_viz({"name": "anything"})
        assert seen["ws_root"] == ws
        assert seen["body"] == {"name": "anything"}
        assert captured["resp"] == {"ok": True, "sentinel": 1}
        assert captured["code"] == 200
