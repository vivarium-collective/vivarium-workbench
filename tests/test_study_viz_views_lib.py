...
# ---------------------------------------------------------------------------
# Server Handler shim parity
# ---------------------------------------------------------------------------

class TestServerShimParity:
    """The real server.Handler methods produce the same output as the lib builders."""

    @staticmethod
    def _invoke_handler(monkeypatch, ws_root: Path, method_name: str, path: str,
                        method_args: tuple = ()):
        """Instantiate server.Handler via __new__, patch _json + path, call the method."""
        ...
        handler.path = path
        getattr(handler, method_name)(*method_args)
        return captured

    def test_visualization_status_missing_name(self, ws, monkeypatch):
        legacy = self._invoke_handler(
            monkeypatch, ws, "_get_visualization_status", "/api/visualization-status"
        )
        lib_body, lib_status = build_visualization_status(ws, "")
        assert legacy["status"] == lib_status == 400
        assert legacy["body"] == lib_body

    def test_visualization_status_missing_viz(self, ws, monkeypatch):
        legacy = self._invoke_handler(
            monkeypatch, ws, "_get_visualization_status",
            "/api/visualization-status?name=nonexistent"
        )
        lib_body, lib_status = build_visualization_status(ws, "nonexistent")
        assert legacy["status"] == lib_status == 200
        assert legacy["body"] == lib_body

    def test_visualization_status_described(self, ws, monkeypatch):
        legacy = self._invoke_handler(
            monkeypatch, ws, "_get_visualization_status",
            "/api/visualization-status?name=my_viz"
        )
        lib_body, lib_status = build_visualization_status(ws, "my_viz")
        assert legacy["status"] == lib_status == 200
        assert legacy["body"] == lib_body

    def test_visualization_instances_parity(self, ws, monkeypatch):
        legacy = self._invoke_handler(
            monkeypatch, ws, "_get_visualization_instances",
            "/api/visualization-instances"
        )
        lib_result = build_visualization_instances(ws)
        assert legacy["status"] == 200
        assert legacy["body"] == lib_result

    def test_study_bigraph_paths_missing_slug(self, study_ws, monkeypatch):
        legacy = self._invoke_handler(
            monkeypatch, study_ws, "_get_study_bigraph_paths",
            "/api/study-bigraph-paths"
        )
        lib_body, lib_status = build_study_bigraph_paths(study_ws, "")
        assert legacy["status"] == lib_status == 400
        assert legacy["body"] == lib_body

    def test_study_bigraph_paths_happy(self, study_ws, monkeypatch):
        legacy = self._invoke_handler(
            monkeypatch, study_ws, "_get_study_bigraph_paths",
            "/api/study-bigraph-paths?study=my-study"
        )
        lib_body, lib_status = build_study_bigraph_paths(study_ws, "my-study")
        assert legacy["status"] == lib_status == 200
        assert legacy["body"] == lib_body

    def test_ptools_launch_no_config(self, tmp_path, monkeypatch):
        """Missing ptools_server_url → 400 from both shim and lib."""
        import vivarium_dashboard.server as server

        # workspace with no ptools_server_url
        (tmp_path / "workspace.yaml").write_text(
            yaml.dump({"name": "ws"}), encoding="utf-8"
        )
        ...
        handler._json = _fake_json  # type: ignore[method-assign]
        handler.path = "/api/ptools-launch/my-study"
        handler.headers = {"Host": "localhost"}  # type: ignore[assignment]
        handler._get_ptools_launch("my-study")

        lib_body, lib_status = build_ptools_launch(tmp_path, "my-study")
        assert captured["status"] == lib_status == 400
        assert captured["body"] == lib_body

    def test_ptools_launch_happy(self, ptools_ws, monkeypatch):
        """Happy path parity: lib and shim return the same body."""
        import vivarium_dashboard.server as server

        monkeypatch.setattr(server, "WORKSPACE", ptools_ws)
        handler = server.Handler.__new__(server.Handler)
        captured: dict = {}
        ...
        handler._json = _fake_json  # type: ignore[method-assign]
        handler.path = "/api/ptools-launch/my-study"
        handler.headers = {"Host": "dash.example.com"}  # type: ignore[assignment]
        handler._get_ptools_launch("my-study")

        lib_body, lib_status = build_ptools_launch(
            ptools_ws, "my-study", public_base="http://dash.example.com"
        )
        assert captured["status"] == lib_status == 200
        assert captured["body"] == lib_body
