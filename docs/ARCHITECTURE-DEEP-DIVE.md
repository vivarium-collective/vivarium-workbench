...
### 5.2 Remote runs (viva-api) — three coexisting implementations
...
2. **Thin-client two-phase** (`/api/remote-run-build|submit|land|poll`): each route is one stateless viva-api call; durability lives in viva-api's Postgres; the JS panel drives the sequence. This is the architecturally correct answer, and comments say it will delete #1 ("R5") — but both still exist.
3. **CLI `run-remote`** (`lib/remote_run.py`): a different viva-api surface (`/compose/*`), exports the composite to `.pbg` with `extra_pip_deps=[git+origin@sha]`, blocks polling.

The good design move: `remote_run_landing.land_remote_run` copies the remote native store (zarr/parquet) *unmodified* to where local chart readers expect it and records a `runs_meta` row, so **a remote run becomes indistinguishable from a local one** to the rest of the app. The viva-api *client* (`sms_api_client.py`) has **no auth** (the security boundary is the SSM tunnel) and is coupled to viva-api's quirks by folklore (response-shape sniffing, `.git`-suffix 500 workarounds, a "required param that doesn't filter"), with a dead `401` handler that can never fire. The deployment name `"smsvpctest"` is hardcoded into durable run provenance.
