...
(function () {
  ...
  function _feedbackWidgetJs(invName, reportId) {
    return '<script>'
      ...
      +     'html+="<button type=\\"button\\" class=\\"fb-bar-btn fb-gh-pr\\" style=\\"margin-left:6px;background:#8250df\\" title=\\"Propose your feedback as a feedback/*.yaml file on a pull request\\">→ GitHub PR</button>";'
      +     'bar.innerHTML=html;'
      ...
      +     'var gp=bar.querySelector(".fb-gh-pr");if(gp)gp.addEventListener("click",function(e){e.preventDefault();e.stopPropagation();openGhPr();});'
      +   '}'
      ...
      +   '}'
      +   'function openGhPr(){if(!countAll()){alert("No feedback yet — click the 💬 icons first.");return;}var repo=ghRepo();if(!repo)return;'
      +     'var fn="feedback/"+INV+"--"+new Date().toISOString().slice(0,10)+".yaml";'
      +     'var url="https://github.com/"+repo+"/new/main?filename="+encodeURIComponent(fn)+"&value="+encodeURIComponent(fbYaml());'
      +     'var w=window.open(url,"_blank","noopener");if(!w)location.href=url;'
      +   '}'
