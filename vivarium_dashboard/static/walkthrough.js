...
(function () {
  ...
  function _buildInvestigationReportHtml(iset, specs, bibEntries, chartsByStudy, embedsByStudy, generation) {
    ...
    return ''
      ...
      // CAN_POST: only a dashboard-served report (http/https) has the
      // /api endpoint. A downloaded report opened over file:// has no
      // server, so we skip the doomed fetch and route the decision into
      // the inline-feedback export instead (pbg-feedback-import applies it).
      +   'var CAN_POST=(location.protocol==="http:"||location.protocol==="https:");'
      +   'function _resolveCard(card,actions,decision,kind,offline){'
      +     'if(!card)return;'
      +     'var accepted=decision==="accept";'
      +     'card.style.borderLeftColor=accepted?"#16a34a":"#dc2626";'
      +     'var msg;'
      +     'if(offline){msg=(accepted?"\\u2713 Accepted":"\\u2717 Declined")+" \\u2014 queued for export. Send your feedback .yaml back to apply it.";}'
      +     'else{msg=accepted?"\\u2713 Accepted by the expert"+(kind==="reference"?" \\u2014 added to the investigation\\u2019s provided references.":" \\u2014 a human integrates the mechanism."):"\\u2717 Declined by the expert \\u2014 not integrated.";}'
      ...
      +   '}'
      +   'function _offlineDecision(itemId,decision,card,actions){'
      +     'if(window._fbRecordDecision){window._fbRecordDecision(itemId,decision);}'
      +     'else{alert("Could not record the decision (feedback widget unavailable).");if(actions){actions.querySelectorAll("button").forEach(function(b){b.disabled=false;});}return;}'
      +     '_resolveCard(card,actions,decision,null,true);'
      +   '}'
      ...
      +     'if(!CAN_POST){_offlineDecision(itemId,decision,card,actions);return;}'
      ...
      +       '_resolveCard(card,actions,newStatus==="accepted"?"accept":"decline",res.body.kind,false);'
      +     '})'
      +     '.catch(function(err){'
      // Server unreachable mid-session → don\'t lose the click; fall back to export.
      +       '_offlineDecision(itemId,decision,card,actions);'
      +     '});'
      +   '};'
      // ── "+ Add to investigation" seed handler (report-scoped) ──────────
      // The SPA defines a richer _seedFollowupProposal that POSTs to
      // /api/study-seed-followup and refreshes the live graph. The DOWNLOADED
      // report has no such function (it lived only in the SPA), so the button
      // used to throw ReferenceError offline. Define a report-scoped version:
      // POST when served by the dashboard, else record a study_seed_request
      // into the inline-feedback export so the offline click is not lost.
      +   'function _seedOffline(parentName,proposalId,btn,title,targets){'
      +     'if(window._fbRecordSeed){window._fbRecordSeed({proposal_id:proposalId,title:title,parent:parentName,targets:targets});}'
      +     'else{alert("Could not record the request (feedback widget unavailable).");if(btn){btn.disabled=false;}return;}'
      +     'if(btn){btn.textContent="\\u2713 queued for export";btn.style.borderColor="#16a34a";btn.style.color="#166534";}'
      +   '}'
      ...
      +     'var title=card?(function(){var t=card.querySelector(".di-fup-title");return t?t.textContent:"";})():"";'
      ...
      +     'if(!CAN_POST){_seedOffline(parentName,proposalId,btn,title,targets);return;}'
      +     'var payload={parent:parentName};if(proposalId)payload.proposal_id=proposalId;payload.proposal_idx=proposalIdx;'
      +     'fetch("/api/study-seed-followup",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(payload)})'
      +     '.then(function(r){return r.json().then(function(d){return {status:r.status,body:d};});})'
      +     '.then(function(res){'
      +       'if(res.status!==200||res.body.error){alert("Seed failed: "+(res.body.error||res.status));if(btn){btn.disabled=false;btn.textContent="\\u2795 Add to investigation";}return;}'
      +       'if(btn){btn.textContent="\\u2713 added: "+(res.body.new_study_name||"new study");btn.style.borderColor="#16a34a";btn.style.color="#166534";}'
      +     '})'
      +     '.catch(function(){_seedOffline(parentName,proposalId,btn,title,targets);});'
      +   '};'
      + '})();'
      + '</script>'
  ...
  function _feedbackWidgetJs(invName, reportId) {
    return '<script>'
      ...
      // loadArr (defined below, hoisted) also reads the decision/seed logs.
      +   'function countAll(){var d=load();var n=0;Object.keys(d).forEach(function(k){n+=(d[k]||[]).length;});n+=loadArr(KEY+"_decisions").length+loadArr(KEY+"_seeds").length;return n;}'
      ...
      +   '}'
      // Offline action log: Accept/Decline on proposed inputs and
      // "Add to investigation" seed requests are recorded here when the
      // report has no dashboard server to POST to (file:// or fetch fails).
      // pbg-feedback-import applies these on import.
      +   'var DKEY=KEY+"_decisions";var SKEY=KEY+"_seeds";'
      +   'function loadArr(k){try{var s=safeGet(k);return s?JSON.parse(s):[];}catch(e){return [];}}'
      +   'function saveArr(k,a){safeSet(k,JSON.stringify(a));}'
      +   'window._fbRecordDecision=function(itemId,decision){var a=loadArr(DKEY);a=a.filter(function(x){return x.item_id!==itemId;});a.push({item_id:itemId,decision:decision,ts:new Date().toISOString()});saveArr(DKEY,a);updateBarCount();};'
      +   'window._fbRecordSeed=function(rec){var a=loadArr(SKEY);a.push({proposal_id:(rec&&rec.proposal_id)||"",title:(rec&&rec.title)||"",parent:(rec&&rec.parent)||"",targets:(rec&&rec.targets)||[],ts:new Date().toISOString()});saveArr(SKEY,a);updateBarCount();};'
      ...
      +     '});}'
      +     'var dec=loadArr(DKEY);'
      +     'if(dec.length){L.push("proposed_input_decisions:");dec.forEach(function(e){L.push("  - item_id: "+JSON.stringify(e.item_id));L.push("    decision: "+JSON.stringify(e.decision));L.push("    ts: "+JSON.stringify(e.ts));});}'
      +     'var seeds=loadArr(SKEY);'
      +     'if(seeds.length){L.push("study_seed_requests:");seeds.forEach(function(e){L.push("  - title: "+JSON.stringify(e.title||""));if(e.proposal_id)L.push("    proposal_id: "+JSON.stringify(e.proposal_id));if(e.parent)L.push("    parent: "+JSON.stringify(e.parent));L.push("    targets: "+JSON.stringify(e.targets||[]));L.push("    ts: "+JSON.stringify(e.ts));});}'
      +     'return L.join("\\n")+"\\n";'
      +   '}'
