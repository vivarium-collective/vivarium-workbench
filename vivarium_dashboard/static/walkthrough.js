...
(function () {
  ...
  function _feedbackWidgetCss() {
    return '<style>'
      ...
      + '.fb-add{position:absolute;top:4px;right:4px;background:#f1f5f9;border:1px solid #cbd5e1;border-radius:999px;width:26px;height:26px;font-size:13px;line-height:1;cursor:pointer;opacity:.35;transition:opacity .15s;z-index:5;padding:0}'
      ...
      + '.fb-editor{margin:8px 0;padding:10px;background:#fffbeb;border:1px solid #fde68a;border-radius:6px}'
      + '.fb-editor textarea{width:100%;box-sizing:border-box;padding:6px;font:inherit;border:1px solid #cbd5e1;border-radius:3px;min-height:80px}'
      + '.fb-editor-row{display:flex;gap:6px;margin-top:6px;align-items:center}'
      + '.fb-editor-row input{flex:1;padding:4px 8px;border:1px solid #cbd5e1;border-radius:3px;font:inherit}'
  ...
  function _feedbackWidgetJs(invName) {
    return '<script>'
      ...
      +       'var btn=document.createElement("button");btn.className="fb-add";btn.title="Add feedback to this section";btn.textContent="💬";'
      +       'btn.onclick=function(e){e.preventDefault();openEditor(el,el.id);};'
      ...
      +              '<button class=\\"fb-del\\" data-i=\\""+i+"\\" title=\\"Delete\\">×</button>'
      ...
      +       'b.onclick=function(){var i=parseInt(b.dataset.i,10);var d=load();(d[sid]||[]).splice(i,1);if(!(d[sid]||[]).length)delete d[sid];save(d);renderExisting(host,sid);updateBadges();updateBarCount();};'
      +     '});'
      +   '}'
      +   'function openEditor(host,sid){'
      +     'if(host.querySelector(":scope>.fb-editor"))return;'
      +     'var ed=document.createElement("div");ed.className="fb-editor";'
      +     'ed.innerHTML="<textarea placeholder=\\"What feedback do you have on this section? (assumption, parameter, evidence, missing detail, etc.)\\"></textarea>'
      ...
      +       '</div>";'
      +     'host.appendChild(ed);'
      +     'setTimeout(function(){ed.querySelector("textarea").focus();},0);'
      +     'ed.querySelector(".fb-cancel").onclick=function(){ed.remove();};'
      +     'ed.querySelector(".fb-save").onclick=function(){'
      ...
      +       'save(d);ed.remove();renderExisting(host,sid);updateBadges();updateBarCount();'
      +     '};'
      +   '}'
      ...
      +     'bar.innerHTML="<button class=\\"fb-bar-btn\\" title=\\"Download all your annotations as a yaml file; send it back to the investigation\\">Generate feedback report <span class=\\"fb-count\\">(0)</span></button>";'
      +     'document.body.appendChild(bar);'
      +     'bar.querySelector(".fb-bar-btn").onclick=downloadFeedback;'
      +   '}'
      +   'function serialiseYaml(meta,data){'
      +     'var L=["# Inline feedback report","# Generated from the v2ecoli inline-feedback widget.","# Import with: pbg-superpowers feedback import <this-file>"];'
      +     'L.push("meta:");'
