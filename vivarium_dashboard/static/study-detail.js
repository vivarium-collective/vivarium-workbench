...
(function() {
  ...
  // ------------------------------------------------------------------
  // Todo #23: auto-render investigation visualizations after pullback
  // ------------------------------------------------------------------
  //
  // When every task's pullback reaches a terminal state (synced or failed)
  // and at least one task actually synced, fire POST /api/investigation-
  // render-viz once.  The render endpoint rebuilds spec.visualizations[*]
  // HTML files against the pulled-back emitter data; report_generator HTML
  // files (todo #22 path) are already on disk and don't need this step,
  // but spec-declared TimeSeriesPlot / Distribution / etc. visualizations do.
  //
  // States are page-local; reload re-arms.  No-viz studies (spec.visualizations
  // empty) report n_visualizations: 0 and we surface that as "no spec viz"
  // rather than an error.
  var _autoRenderState = 'idle';  // idle | in_flight | rendered | failed | skipped
  var _autoRenderError = null;
  var _autoRenderResult = null;

  function maybeAutoRenderReport() {
    if (_autoRenderState !== 'idle') return;  // dedupe — fire once per page load
    // Need the latest array status to know the *total* task count.  Without
    // it, a single synced task in _taskState would prematurely satisfy the
    // "all terminal" check while other tasks are still RUNNING / PENDING.
    if (!_lastArrayStatus || !Array.isArray(_lastArrayStatus.tasks)) return;
    var arrTasks = _lastArrayStatus.tasks;
    var arrRunIds = _lastArrayStatus.run_ids || [];
    if (arrTasks.length === 0) return;
    // Every array task must (a) be in a terminal SLURM state and (b) have a
    // terminal pullback state.  Tasks the user never triggered pullback on
    // (FAILED tasks where no auto-pullback fired) count as "no pullback to
    // wait on" — treat them as terminal.
    var anySynced = false;
    for (var i = 0; i < arrTasks.length; i++) {
      var t = arrTasks[i];
      var taskState = t.state || '';
      var terminalSlurm = (taskState === 'COMPLETED' || taskState === 'FAILED' ||
                           taskState === 'CANCELLED' || taskState === 'TIMEOUT');
      if (!terminalSlurm) return;
      var runId = arrRunIds[i] || t.run_id;
      var s = (runId && _taskState[runId]) || null;
      var pullbackTerminal = (!s || s.pullback === 'synced' || s.pullback === 'failed');
      if (!pullbackTerminal) return;
      if (s && s.pullback === 'synced') anySynced = true;
    }
    if (!anySynced) {
      // All pullbacks failed — no point rendering.  Surface that.
      _autoRenderState = 'skipped';
      _autoRenderError = 'all task pullbacks failed';
      renderAutoRenderChip();
      return;
    }
    _autoRenderState = 'in_flight';
    renderAutoRenderChip();
    api('POST', '/api/investigation-render-viz', { name: STUDY_NAME })
      .then(function(r) {
        if (r.status === 200 && r.body && r.body.ok) {
          _autoRenderState = 'rendered';
          _autoRenderResult = r.body;
        } else {
          _autoRenderState = 'failed';
          _autoRenderError = (r.body && r.body.error) || ('HTTP ' + r.status);
        }
        renderAutoRenderChip();
      })
      .catch(function(err) {
        _autoRenderState = 'failed';
        _autoRenderError = String(err);
        renderAutoRenderChip();
      });
  }

  function renderAutoRenderChip() {
    var box = document.getElementById('hpc-auto-render-chip');
    if (!box) return;
    var html = '';
    if (_autoRenderState === 'idle') {
      box.style.display = 'none';
      return;
    }
    box.style.display = '';
    if (_autoRenderState === 'in_flight') {
      html = '<span style="color:#2563eb;font-size:0.85em">⟳ rendering report…</span>';
    } else if (_autoRenderState === 'rendered') {
      var n = (_autoRenderResult && _autoRenderResult.n_visualizations) || 0;
      if (n === 0) {
        html = '<span style="color:#64748b;font-size:0.85em">' +
               '✓ all tasks synced — no spec-declared visualizations to render ' +
               '(report_generator outputs, if any, are in the Visualizations tab)' +
               '</span>';
      } else {
        html = '<span style="color:#16a34a;font-size:0.85em">' +
               '✓ report rendered (' + n + ' visualization' + (n === 1 ? '' : 's') +
               ') — <a href="#visualizations" onclick="window.location.reload()" ' +
               'style="color:#16a34a;text-decoration:underline">view</a>' +
               '</span>';
      }
    } else if (_autoRenderState === 'failed') {
      var err = (_autoRenderError || 'unknown error').replace(/</g, '&lt;');
      html = '<span style="color:#dc2626;font-size:0.85em" title="' + err + '">' +
             '✗ report render failed — ' +
             '<button id="hpc-auto-render-retry" style="font-size:0.85em;padding:1px 8px">retry</button>' +
             '</span>';
    } else if (_autoRenderState === 'skipped') {
      html = '<span style="color:#d97706;font-size:0.85em">' +
             '⚠ skipped report render — ' + (_autoRenderError || 'no synced tasks') +
             '</span>';
    }
    box.innerHTML = html;
    var retry = document.getElementById('hpc-auto-render-retry');
    if (retry) {
      retry.addEventListener('click', function() {
        _autoRenderState = 'idle';
        _autoRenderError = null;
        maybeAutoRenderReport();
      });
    }
  }
