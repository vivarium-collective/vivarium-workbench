...
- **2026-07-13 (executing)**: WS-A/B/C DONE + pushed. WS-A: `Dockerfile` bigraph-loom overlay + broadened sanity import; audit cleared `pbg_basic_processes` (declared-but-unused). WS-B: allowlist across `csrf.py`/`env_compat.py`/`api/app.py`/`cli.py`/`startup.py`, **33 tests pass**; **live probe CONFIRMED the ALB rewrites Host** (403 even with matching client Host) so `--trust-proxy` is dead → allowlist is the fix (see [[project_alb_rewrites_host_csrf]]). WS-C: WALKTHROUGH remote-first, local flow → Appendix G. Pushed: dashboard `demo-v2ecoli` `2c56cb8`, sms-api `patch/db-filter` `15c3391` (base env `VIVARIUM_WORKBENCH_ALLOWED_ORIGINS=http://localhost:8080`). WS-D: image building (`gh run 29282389101`, tag `2c56cb8`); overlay `newTag` bump + `kubectl apply -k` pending build. WS-E/F not started. Signing disabled for these commits (user-authorized once).
